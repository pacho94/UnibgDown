...
async function downloadVideo(videoUrls, username, password, outputDirectory) {
   ...
   if(password === undefined) { // password not passed as argument
        ...
        if(argv.noKeyring === false) {
          try {
              await keytar.getPassword("UnibgDown", username).then(function(result) { password = result; });
              if (password === null) { // no previous password saved
                  password = await promptQuestion("Password not saved. Please enter your password, UnibgDown will not ask for it next time: ");
                  await keytar.setPassword("UnibgDown", username, password);
              } else {
                  console.log("Reusing password saved in system's keychain!")
              }
          }
          catch(e) {
              console.log("X11 is not installed on this system. UnibgDown can't use keytar to save the password.")
   ...
   const usernameEmail = username + "@studenti.unibg.it";
   ...
   await browser.waitForTarget(target => target.url().includes('microsoftstream.com/'), { timeout: 9000 });
